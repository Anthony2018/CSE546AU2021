...
def vanilla_add(a: Vector, b: Vector) -> Vector:
    """Performs a matrix add
...
def vanilla_add_matrix(A: Matrix, B: Matrix) -> Matrix:
    """Performs a matrix add
...
def vanilla_solution(x: Vector, y: Vector, A: Matrix, B: Matrix) -> Vector:
    ...
    """
    add_A_matrix = vanilla_add_matrix(A, vanilla_transpose(A))
    g_f_x_y = vanilla_add(vanilla_matmul(add_A_matrix, x) , vanilla_matmul(vanilla_transpose(B), y))
    return g_f_x_y
...
def numpy_solution(
    ...
    """
    g_f_x_y = np.dot((A + np.transpose(A)), x) + np.dot(np.transpose(B) , y)
    return g_f_x_y
