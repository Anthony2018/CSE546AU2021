...
class SGDOptimizer(torch.optim.Optimizer):
    ...
    def step(self, closure=None):  # noqa: E251
        ...
        """
        # for parameter_dic in self.param_groups:
        #     print(parameter_dic['params'])
        #     # for parameter in parameter_dic:
        #     #     print(parameter)
        #     #     parameter -= self.lr * parameter
        for data in self.param_groups:
            for i in range(len(data['params'])):
                data['params'][i].data = data['params'][i].data - self.lr * data['params'][i].grad
